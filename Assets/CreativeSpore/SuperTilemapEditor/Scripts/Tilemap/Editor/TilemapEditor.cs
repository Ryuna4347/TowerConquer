    ...
    public partial class TilemapEditor : Editor
    {
        ...
        private void OnInspectorGUI_Map()
        {
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                EditorGUILayout.PropertyField(serializedObject.FindProperty("mapLev"), new GUIContent("Level"));
        ...
        private void OnInspectorGUI_Renderer()
        {
            ...
            */
            ...
            if(GUILayout.Button("Export To Png file"))
            {
                ...
                {
                    Texture2D previewTexture = TilemapUtilsEditor.CreateTilemapGroupPreviewTexture(m_tilemap, m_tilemap.ParentTilemapGroup);
        ...
        //에디터를 통해 그렸던 맵을 xml형태로 저장
        public void SaveMapData()
        {
            int mapLev = m_tilemap.GetMapLev();

            int width = m_tilemap.MaxGridX - m_tilemap.MinGridX + 1;
            ...
            {
                prevJson[i] = JsonUtility.FromJson<MapJSON>(prevMapData[i]);
            }
            
            int len = (prevJson == null) ? 1 : prevLen+1;
            MapJSON[] mapJson=new MapJSON[len]; //+1은 지금 집어넣을 애가 혹시 업데이트가 아닌 추가일 수 있으므로

            
            for (int y = m_tilemap.MinGridY; y <= m_tilemap.MaxGridY; y++)
            {
                for (int x = m_tilemap.MinGridX; x <= m_tilemap.MaxGridX; x++)
                {
                    uint tileData = m_tilemap.GetTileData(x, y);
                    int tileId = (int)(tileData & Tileset.k_TileDataMask_TileId);
                    mapDataId[(y + (height / 2)) * width + (x + (width / 2))] = tileId;
                }
            }

            int j = 0; //prevJson의 인덱스 검색할때 사용(바로 아래)
            if (mapLev != 0 && (mapDataId != null))
            { //입력할 데이터들이 모두 존재한다면
                for (int i = 0; i < len; i++)
                {
                    bool newDataInserted = false;
                    if ((i<prevJson.Length)&&(prevJson.Length!=0))
                    { //우선 prevJson보다 크거나 prevJson이 없다면 새로운 데이터가 들어가게 된다.
                    ...
                    if (newDataInserted)
                    { //이전 데이터를 무시하고 현재 데이터를 삽입
                        MapJSON temp = new MapJSON();

                        temp.tileSetName = m_tilemap.Tileset.name;
                        temp.level = mapLev;
                        temp.mapData = mapDataId;
                        mapJson[mapLev - 1] = temp; //-1인 이유: mapLev은 1부터, 배열은 0부터 시작하기 때문에

                    }
                    else {
                        mapJson[i] = prevJson[j++]; //기존 데이터 갈아끼기
                    }
                }
            }
            ...
            {
                json += JsonUtility.ToJson(mapJson[i]); //mapJson전체를 한번에 json화 시킬수는 없나보다.
                if (i != (mapJson.Length - 1) &&(json!=""))
                {//마지막까지 포함시켜서 엔터를 넣으면 실행할때마다 계속 넣게됨
                    json += "\n";
                }
            }
        ...
        public void LoadMapData()
        {
            ...
            {  //저장된 정보가 있어야 불러옴
                ...
                for (int i = 0; i < prevLen; i++)
                {
                    ...
                    prevJson[i] = JsonUtility.FromJson<MapJSON>(prevMapData[i]);
                    Debug.Log(i+" : "+ prevJson[i].level);
                    if (prevJson[i].level == mapLev)
                    { //로드하려는 맵과 같을 경우에는 따로 저장해둘것
                        mapDataId = prevJson[i].mapData;
                    }
                }

                int[,] loadedMapData = new int[width, height];

                //1차원배열->2차원배열
                for (int i = 0; i < mapDataId.Length; i++)
                {
                    int y = i / width;
                    int x = i - y * width;
                    loadedMapData[x, y] = mapDataId[i];

                    TileData tData = new TileData();
                    tData.tileId = loadedMapData[x, y];

                    m_tilemap.SetTileData(m_tilemap.MinGridX + x, m_tilemap.MinGridY + y, tData.BuildData());
                }
                //setMapData
