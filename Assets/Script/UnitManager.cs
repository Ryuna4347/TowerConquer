...
public class UnitManager : MonoBehaviour
{
    ...
    public void InstallUnit()
    {
        ...
        GameObject useUnit = unusedDefUnitList[installUnit]; //사용할 유닛 설정
        useUnit.SetActive(true); //아래의 돈확인을 위해서는 스크립트를 사용해야하므로 켜야한다.

        //돈관련
        int money = stageManager.GetMoney(); //현재 유저의 잔고
        int requiredMoney = useUnit.GetComponent<UnitBase>().GetInstallCost();
        ...
        stageManager.UseMoney(requiredMoney); //돈 차감
